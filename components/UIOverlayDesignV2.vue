...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const remainingToReward = computed(() => {
      ...
      return Math.max(0, nextReward.value.score - props.score.current);
    });

    const earnedReward = computed<PromoReward | null>(() => {
      const copy = [...PROMO_REWARDS].reverse();
      return copy.find((reward) => reward.score <= props.score.current) || null;
    });
    ...
    const segmentProgress = (index: number) => {
      ...
      if (props.score.current <= segmentStart) {
        return 0;
      }

      if (props.score.current >= segmentEnd) {
        return 1;
      }
    ...
    const emitRestart = () => emit('restart');
    const emitOpenLeaderboard = () => emit('open-leaderboard');
    const emitCloseLeaderboard = () => emit('close-leaderboard');
    ...
    const donation = computed(() => props.score.current);
